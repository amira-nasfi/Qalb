...
class ReportDetailView(generics.RetrieveUpdateAPIView):
    ...
    """
    queryset = Report.objects.select_related("result__record__patient", "signed_by").all()
    permission_classes = [permissions.IsAuthenticated]

    def get_object(self):
        obj = super().get_object()
        user = self.request.user
        if getattr(user, 'is_field_agent', False) and obj.status != Report.Status.SIGNED:
            self.permission_denied(self.request, message="Field agents can only view signed reports.")
        return obj
...
class ReportSignView(APIView):
    ...
    def post(self, request, pk):
        ...
        if report.status == Report.Status.SIGNED:
            return Response({"error": "Report is already signed"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if not report.physician_notes.strip():
            return Response({"error": "Physician notes cannot be empty before signing"}, status=status.HTTP_400_BAD_REQUEST)

        report.status = Report.Status.SIGNED
        report.signed_by = request.user
        report.signed_at = timezone.now()
        report.save(update_fields=["status", "signed_by", "signed_at", "updated_at"])
