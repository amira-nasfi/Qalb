...
def test_role_based_access(api_client, create_user):
    ...
    # Physician shouldn't access admin
    ...
    # Field agent shouldn't access fhir export (which is phys/admin only)
    api_client.force_authenticate(user=field_agent)
    url = reverse("fhir_export:report_export", kwargs={"report_id": 1})
    resp = api_client.get(url)
    # We expect 403, or 404 if report doesn't exist but permissions are checked first.
    # Actually DRF checks permissions before view object get, so it returns 403.
    assert resp.status_code == status.HTTP_403_FORBIDDEN
